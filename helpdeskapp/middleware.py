...
class LoginRequiredMiddleware:
    ...
    def __call__(self, request):
        ...
        def safe_reverse(name, default_path):
            ...
            try:
                ...
                try:
                    ...
                    # Remplacez 'app_name' par le namespace réel de votre app d'auth si besoin
                    url = reverse(f'auth:{name}')
                    return url if url.endswith('/') else url + '/'
                except NoReverseMatch:
                    # Retourne le chemin par défaut en dernier recours
                    return default_path if default_path.endswith('/') else default_path + '/'

        # --- 2. Définition des URLs de base ---
        
        # NOTE: Assurez-vous que 'login', 'register' et 'logout' sont bien les noms dans vos urls.py
        ...
        password_reset_url = safe_reverse('password_reset_request', '/password-reset/')
        
        # --- 3. Définition des URLs publiques (Accessibles sans connexion) ---
        
        # Seules ces pages sont accessibles si l'utilisateur n'est pas connecté.
        public_urls = [
        ...
        # Une page est privée si elle n'est pas dans la liste public_urls ET si ce n'est pas un fichier statique
        if not user_is_logged_in and not is_public and not request_path.startswith('/static/'):
