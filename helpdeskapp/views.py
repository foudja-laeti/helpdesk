...
def get_weather_data(city="Douala"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(
                f"API météo erreur {
                    response.status_code} - Fallback données factices")
            return {
                'city': city,
                'temperature': 28,
                'description': 'Partiellement nuageux',
...
def add_demande(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            logger.info(
                f"Nouvelle demande ajoutée par l'utilisateur ID: {utilisateur_id}, Demande ID: {
                    demande.id}")
...
def register_view(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            # Point de redirection CLÉ 2: Après inscription réussie, va à
            # 'login'
            return redirect('login')
        else:
            messages.error(
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if utilisateur:
                ...
                # Pour des raisons de sécurité, on ne dit pas que l'email
                # n'existe pas
                messages.info(
                    request, 'Si cet email existe, un lien de réinitialisation sera généré.')
